 ...
 * @author Admin
 */
public class ModifierClient extends javax.swing.JDialog {

    private static final java.util.logging.Logger logger = java.util.logging.Logger.getLogger(ModifierClient.class.getName());

    private int idClientCourant;

    public ModifierClient(java.awt.Frame parent, boolean modal, int idClient) {
        super(parent, modal);
        initComponents();
        setLocationRelativeTo(parent);
        idClientCourant = idClient;
        configurerStyle();
        chargerDonneesClient(idClient);
        btnAnnuler.addActionListener(e -> dispose());
        configurerListenersFormatage();
        txtDateNaissance.setMaxSelectableDate(new java.util.Date());
    }

    private void configurerListenersFormatage() {
        ...
        txtTelephone.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyReleased(java.awt.event.KeyEvent e) {
                ...
                if (kc == java.awt.event.KeyEvent.VK_BACK_SPACE
                        || kc == java.awt.event.KeyEvent.VK_DELETE) {
                    return;
                }
    ...
    private void configurerStyle() {
        ...
        btnMettreAJour.setBackground(new java.awt.Color(220, 252, 231));
        btnMettreAJour.setForeground(new java.awt.Color(21, 128, 61));
        btnMettreAJour.setBorderPainted(false);
        btnMettreAJour.setFocusPainted(false);
        btnMettreAJour.setCursor(new java.awt.Cursor(java.awt.Cursor.HAND_CURSOR));
    ...
    private void chargerDonneesClient(int idClient) {
        ...
        txtAdresse.setText(c.getAdresse() != null ? c.getAdresse() : "");
        txtNumeroPiece.setText(c.getNumCarteIdentite() != null
                ? c.getNumCarteIdentite() : "");
        txtProfession.setText(c.getProfession() != null
                ? c.getProfession() : "");
        if (c.getRevenuMensuel() > 0) {
            txtRevenu.setText(String.format("%.0f", c.getRevenuMensuel()));
        }
        ...
        cmbSexe.setSelectedItem(c.getSexe());
        if (c.getSituationMat() != null) {
            cmbSituationMat.setSelectedItem(c.getSituationMat());
        }
        if (c.getTypePiece() != null) {
            cmbTypePiece.setSelectedItem(c.getTypePiece());
        }
    }
    ...
    private boolean validerChamps() {
        ...
        if (txtNumeroPiece.getText().trim().isEmpty()) {
            afficherErreur("Le numéro de pièce est obligatoire !");
            txtNumeroPiece.requestFocus();
            return false;
        }
    ...
    afficherErreur("Le revenu mensuel est obligatoire !");
    txtRevenu.requestFocus();
    return false;
}
try {
    double r = Double.parseDouble(
        txtRevenu.getText().trim().replace(" ", ""));
    if (r <= 0) {
        afficherErreur("Le revenu doit être supérieur à 0 !");
        txtRevenu.requestFocus();
        return false;
    }
} catch (NumberFormatException ex) {
    afficherErreur("Le revenu doit être un nombre !");
    txtRevenu.requestFocus();
    return false;
}
    ...
    return false;
        ...
        if (txtDateNaissance.getDate() != null) {
            long age = (new java.util.Date().getTime() - txtDateNaissance.getDate().getTime())
                    / (1000L * 60 * 60 * 24 * 365);
            if (age < 18) {
                afficherErreur("Le client doit avoir au moins 18 ans !");
                return false;
            }
            if (age > 100) {
                afficherErreur("La date de naissance semble incorrecte !");
                return false;
            }
        }
        return true;
    ...
    private void initComponents() {
        ...
        lblDialogTitle.setText("Modifier client");
        headerLeft.add(lblDialogTitle);

        lblDialogSub.setForeground(new java.awt.Color(100, 116, 139));
        lblDialogSub.setText("Modifiez les informations du client");
        ...
        btnAnnuler.setMaximumSize(new java.awt.Dimension(100, 36));
        ...
        btnMettreAJour.setPreferredSize(new java.awt.Dimension(130, 36));
        btnMettreAJour.addActionListener(this::btnMettreAJourActionPerformed);
        footerBtns.add(btnMettreAJour);
...
if (confirm != javax.swing.JOptionPane.YES_OPTION) return;
        ...
        // 2. Unicité téléphone — exclure le client courant
        if (dao.telephoneExiste(txtTelephone.getText().trim(), idClientCourant)) {
            afficherErreur("Ce numéro de téléphone est déjà utilisé par un autre client !");
            txtTelephone.requestFocus();
            return;
        }
        // Doublon nom+prénom
        java.sql.Date dn2 = txtDateNaissance.getDate() != null
                ? new java.sql.Date(txtDateNaissance.getDate().getTime()) : null;
        if (dao.clientDoublonExiste(txtNom.getText().trim(),
                txtPrenom.getText().trim(), dn2, idClientCourant)) {
            afficherErreur("Un autre client avec ce nom et prénom existe déjà !");
            return;
        }

        // 3. Unicité numéro de pièce — exclure le client courant
        if (dao.numeroPieceExiste(txtNumeroPiece.getText().trim(), idClientCourant)) {
            afficherErreur("Ce numéro de pièce est déjà enregistré !");
            txtNumeroPiece.requestFocus();
            return;
        }
        ...
        client.setIdClient(idClientCourant);
        ...
        if (txtDateNaissance.getDate() != null) {
            client.setDateNaissance(new java.sql.Date(
                    txtDateNaissance.getDate().getTime()));
        }
        if (!txtRevenu.getText().trim().isEmpty()) {
            client.setRevenuMensuel(Double.parseDouble(
                    txtRevenu.getText().trim().replace(" ", "")));
        }

        // 5. Mettre à jour
        if (dao.modifier(client)) {
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            @Override
            public void run() {
                ModifierClient dialog = new ModifierClient(new javax.swing.JFrame(), true, 0);
